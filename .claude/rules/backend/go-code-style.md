...
- Repository 系 interface は対応する実装 struct の名称に `-er` サフィックスを付ける（例: 実装 `BookQueryRepository` → interface `BookQueryRepositorier`、実装 `BookCommandRepository` → interface `BookCommandRepositorier`）。Repository 以外の汎用 interface（リーダー・クローザー等）は通常の Go 慣用 `-er` を維持してよい
